...
class CADQueryInterface:
    ...
    def get_connected_blocks_of_type(self, block_id: str, block_type: str, 
                                    direction: str = "both", threshold: float = 0.7) -> List[Block]:
        """
        获取与指定块连接的指定类型的所有块
    ...
    def find_path(self, source_id: str, target_id: str) -> List[Block]:
        ...
        return [self.graph.blocks[block_id] for block_id in path_ids if block_id in self.graph.blocks]
    ...
    def find_all_paths(self, source_id: str, target_id: str, 
        ...
        for path in path_lists:
            block_path = [self.graph.blocks[block_id] for block_id in path if block_id in self.graph.blocks]
            ...
            for key, condition in criteria.items():
                ...
                if isinstance(condition, tuple) and len(condition) == 2:
                    ...
                    if op == '>' and not (value > target):
                        ...
                        match = False
                        break
                    elif op == 'contains' and not (target in value if isinstance(value, str) else False):
                        match = False
                        break
                    elif op == 'startswith' and not (value.startswith(target) if isinstance(value, str) else False):
                        match = False
                        break
                    elif op == 'endswith' and not (value.endswith(target) if isinstance(value, str) else False):
                        match = False
                        break
                else:
                    # 直接比较相等
    ...
    def query(self, query_str: str) -> Dict:
        ...
        try:
            # 入度查询
            if re.search(r'(indegree|in degree|incoming|in connections) .* (block|node) ([a-zA-Z0-9_]+)', query_str):
                match = re.search(r'(indegree|in degree|incoming|in connections) .* (block|node) ([a-zA-Z0-9_]+)', query_str)
            ...
            # 出度查询
            elif re.search(r'(outdegree|out degree|outgoing|out connections) .* (block|node) ([a-zA-Z0-9_]+)', query_str):
                match = re.search(r'(outdegree|out degree|outgoing|out connections) .* (block|node) ([a-zA-Z0-9_]+)', query_str)
                block_id = match.group(3)
            ...
            # 连接查询
            elif re.search(r'(has connection|is connected|connection exists) .* (from|between) ([a-zA-Z0-9_]+) .* (to|and) ([a-zA-Z0-9_]+)', query_str):
                match = re.search(r'(has connection|is connected|connection exists) .* (from|between) ([a-zA-Z0-9_]+) .* (to|and) ([a-zA-Z0-9_]+)', query_str)
            ...
            # 路径查询
            elif re.search(r'(path|route|connection) .* (from|between) ([a-zA-Z0-9_]+) .* (to|and) ([a-zA-Z0-9_]+)', query_str):
                match = re.search(r'(path|route|connection) .* (from|between) ([a-zA-Z0-9_]+) .* (to|and) ([a-zA-Z0-9_]+)', query_str)
            ...
            # 查找特定类型的块
            elif re.search(r'find .* (blocks?|nodes?) .* (type|kind|category) ([a-zA-Z0-9_]+)', query_str):
                match = re.search(r'find .* (blocks?|nodes?) .* (type|kind|category) ([a-zA-Z0-9_]+)', query_str)
...
class CADQueryResults:
    ...
    def format_block_info(block: Block) -> Dict:
        ...
        if block.reference:
            info['reference'] = {
                ...
                'position': (block.reference.position.x, block.reference.position.y, block.reference.position.z),
    ...
    def format_connection_info(connection: Connection) -> Dict:
        ...
        info['segments'] = [
            {
                'id': segment.id,
                'start_point': (segment.start_point.x, segment.start_point.y, segment.start_point.z),
                'end_point': (segment.end_point.x, segment.end_point.y, segment.end_point.z),
    ...
    def format_query_results(results: Dict) -> Dict:
        ...
        if data is not None:
            ...
            if isinstance(data, list) and data and isinstance(data[0], Block):
                formatted['data'] = [CADQueryResults.format_block_info(block) for block in data]
            ...
            # 格式化连接列表
                ...
                formatted['data'] = [CADQueryResults.format_connection_info(conn) for conn in data]
            ...
            # 格式化路径
            elif isinstance(data, list) and data and isinstance(data[0], list) and data[0] and isinstance(data[0][0], Block):
                formatted['data'] = [
