...
def analyze_spatial_layout(system):
    ...
    print(f"边界范围: X: {bounds.get('min_x', 0):.2f} - {bounds.get('max_x', 0):.2f}, "
...
def analyze_paths_and_cycles(system):
    ...
    if 'strongly_connected_components' in connectivity:
        print(f"  强连通分量数量: {connectivity.get('strongly_connected_components', 0)}")
...
def visualize_graph(system):
    ...
    try:
        ...
        # 绘制节点
        nx.draw_networkx_nodes(G, positions, node_size=300, node_color=node_color_list, alpha=0.8)
        ...
        for (u, v, data) in G.edges(data=True):
            ...
            if data.get('explicit_direction', False):
                ...
                nx.draw_networkx_edges(G, positions, edgelist=[(u, v)], 
                                      arrows=False, 
                                      edge_color=edge_colors.get((u, v), 'k'),
                                      style='dashed' if data.get('connection_type', '') == 'indirect' else 'solid',
                                      width=1.5,
                                      alpha=0.6)
        ...
        for i, block in enumerate(blocks[:5], 1):  # 只显示前5个
            ...
            if i <= 3:  # 只详细分析前3个
                ...
                for conn in in_connections:
                    block_matches = system.block_identifier.identify_block(conn.source_block)
                ...
                for conn in out_connections:
                    block_matches = system.block_identifier.identify_block(conn.target_block)
                    block_type = block_matches[0][0] if block_matches else "unknown"
                    connected_types[block_type] = connected_types.get(block_type, 0) + 1
                
                print("   连接的块类型:")
                for block_type, count in sorted(connected_types.items(), key=lambda x: x[1], reverse=True):
                    print(f"     {block_type}: {count}")
