...
class ConnectionAnalyzer:
    ...
    def _are_segments_connected(self, line1: LineEntity, line2: LineEntity) -> bool:
        ...
        distances = [
            (
                line1.start_point.distance_to(line2.start_point),
                (line1.start_point, line2.start_point),
            ),
            (
                line1.start_point.distance_to(line2.end_point),
                (line1.start_point, line2.end_point),
            ),
            (
                line1.end_point.distance_to(line2.start_point),
                (line1.end_point, line2.start_point),
            ),
            (
                line1.end_point.distance_to(line2.end_point),
    ...
    def _is_arrow_on_segment(self, arrow_block: Block, segment: LineEntity) -> bool:
        ...
        return (
            self._point_line_distance(
                arrow_block.center, segment.start_point, segment.end_point
            )
            < 5.0
        )
    ...
    def _get_arrow_direction(self, arrow_block: Block) -> Tuple[float, float, float]:
        ...
        if aspect > 1.0:  # 宽大于高
            ...
            midpoint_x = (
                arrow_block.bounding_box.min_point.x
                + arrow_block.bounding_box.max_point.x
            ) / 2
            ...
            midpoint_y = (
                arrow_block.bounding_box.min_point.y
                + arrow_block.bounding_box.max_point.y
            ) / 2
...
class ConnectionClassifier:
    ...
    def get_connection_stats(self, connections: List[Connection]) -> Dict:
        ...
        """
        if not connections:
            return {"count": 0, "message": "No connections to analyze"}
        ...
        avg_segments = (
            sum(segment_counts) / len(segment_counts) if segment_counts else 0
        )
