...
class EntityRenderer:
    ...
    def render_entity(self, entity: Entity, ax: plt.Axes, **kwargs):
        ...
        # 处理点类型的特殊情况
        if entity_type == 'POINT' or (hasattr(entity_type, 'value') and entity_type.value == 'POINT'):
    ...
    def render_line(self, entity: Any, ax: plt.Axes, 
                   color: str = '#ff7f0e', linewidth: float = 1.0, 
                   linestyle: str = '-', alpha: float = 1.0, zorder: int = 5, **kwargs):
        """
        渲染直线
    ...
    def render_circle(self, entity: Any, ax: plt.Axes, 
                     color: str = '#2ca02c', linewidth: float = 1.0, 
                     linestyle: str = '-', alpha: float = 1.0, zorder: int = 5, **kwargs):
        """
        渲染圆
    ...
    def render_arc(self, entity: Any, ax: plt.Axes, 
        ...
        Args:
            ...
            **kwargs: 其他参数
        """
        if not hasattr(entity, 'center') or not hasattr(entity, 'radius') or \
           not hasattr(entity, 'start_angle') or not hasattr(entity, 'end_angle'):
    ...
    def render_ellipse(self, entity: Any, ax: plt.Axes, 
                      color: str = '#d62728', linewidth: float = 1.0, 
                      linestyle: str = '-', alpha: float = 1.0, zorder: int = 5, **kwargs):
        """
        渲染椭圆
    ...
    def render_text(self, entity: Any, ax: plt.Axes, 
                   color: str = '#9467bd', alpha: float = 1.0, 
                   linewidth: float = None, linestyle: str = None, zorder: int = 10, **kwargs):
        """
        渲染文本
        ...
        # 计算旋转角度（弧度转角度）
        angle_degrees = math.degrees(entity.rotation) if hasattr(entity, 'rotation') else 0
    ...
    def render_spline(self, entity: Any, ax: plt.Axes, 
                     color: str = '#8c564b', linewidth: float = 1.0, 
                     linestyle: str = '-', alpha: float = 1.0, zorder: int = 5, **kwargs):
        """
        渲染样条曲线
    ...
    def render_polyline(self, entity: Any, ax: plt.Axes, 
                       color: str = '#e377c2', linewidth: float = 1.0, 
                       linestyle: str = '-', alpha: float = 1.0, zorder: int = 5, **kwargs):
        """
        渲染多段线
    ...
    def render_point(self, entity: Any, ax: plt.Axes, 
                    color: str = '#7f7f7f', size: float = 5.0, 
                    alpha: float = 1.0, zorder: int = 5, **kwargs):
        """
        渲染点
    ...
    def render_hatch(self, entity: Any, ax: plt.Axes, 
                    color: str = '#bcbd22', alpha: float = 0.3):
        """
        渲染填充
    ...
    def render_unknown(self, entity: Any, ax: plt.Axes,
        ...
        if hasattr(entity, 'position'):
            # 如果有位置属性，渲染为点
            self.render_point(entity, ax, color=color, size=3.0, alpha=alpha, zorder=zorder)
