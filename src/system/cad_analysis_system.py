...
from core.data_structures import (
    Entity, Block, Connection, BlockFeature, LineEntity
)
...
class CADAnalysisSystem:
    ...
    def analyze_file(self, file_path: str) -> bool:
        ...
        try:
            ...
            # 解析文件
            self.entities, self.blocks, self.additional_info = parser.parse_file(file_path)
            
            # 提取线段用于连接分析
            lines = [entity for entity in self.entities if isinstance(entity, LineEntity)]
            
            # 查找块之间的连接
            self.connections = self.connection_analyzer.find_connections(self.blocks, lines)
    ...
    def create_block_feature_from_block(self, block: Block, name: str, description: str = "", tolerance: float = 0.2) -> Optional[BlockFeature]:
        """
        从块创建特征
        ...
        """
        try:
            feature = BlockFeature.from_sample_block(block, name, description, tolerance)
            return feature
        except Exception as e:
            print(f"从块创建特征时出错: {e}")
            return None
    ...
    def set_connection_parameters(self, max_gap_distance: float = None, 
    ...
    def get_statistics(self) -> Dict:
        ...
        stats = {
            ...
            "line_count": sum(1 for entity in self.entities if isinstance(entity, LineEntity)),
...
class CADAnalysisConfig:
    ...
    def apply_to_system(self, system: CADAnalysisSystem):
        ...
        system.set_connection_parameters(
            ...
            connection_angle_tolerance=self.connection_params.get("connection_angle_tolerance"),
            block_connection_tolerance=self.connection_params.get("block_connection_tolerance")
        )
