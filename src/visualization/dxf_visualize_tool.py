...
def parse_args():
    ...
    # 可选参数
    parser.add_argument('-o', '--output', type=str, help='输出图像文件路径', default=None)
    parser.add_argument('--no-display', action='store_true', help='不显示可视化结果，只保存图像')
    ...
    # 内容控制
    parser.add_argument('--no-blocks', action='store_true', help='不显示块边界和内部结构')
    parser.add_argument('--no-labels', action='store_true', help='不显示块名称标签')
    parser.add_argument('--block-mode', type=str, choices=['boundary', 'structure'], default='structure',
                      help='块显示模式: boundary(只显示边界框) 或 structure(显示完整内部结构)')
    parser.add_argument('--show-connections', action='store_true', help='显示连接关系')
    parser.add_argument('--highlight', type=str, help='高亮显示指定名称的块', default=None)
    parser.add_argument('--layer', type=str, help='仅显示指定的图层（多个图层用逗号分隔）', default=None)
    
    # 视图控制
    parser.add_argument('--no-grid', action='store_true', help='不显示网格')
    parser.add_argument('--no-axis', action='store_true', help='不显示坐标轴')
    parser.add_argument('--figsize', type=str, help='图像尺寸，格式为"宽度,高度"，单位为英寸', default='12,8')
    parser.add_argument('--dpi', type=int, help='图像分辨率', default=100)
    parser.add_argument('--view-range', type=str, help='视图范围，格式为"min_x,min_y,max_x,max_y"', default=None)
    parser.add_argument('--margin', type=float, help='视图边距', default=20)
    parser.add_argument('--auto-scale', action='store_true', help='自动根据内容调整视图比例')
    ...
    parser.add_argument('--highlight-color', type=str, help='高亮颜色', default='#d62728')
...
def main():
    ...
    try:
        ...
        if args.debug:
            ...
            if blocks and len(blocks) > 0:
                ...
                for i, block in enumerate(blocks[:5]):  # 只打印前5个
                    print(f"  块 {i+1}: {block.name} (含 {len(block.entities) if hasattr(block, 'entities') else 0} 个实体)")
                if len(blocks) > 5:
        ...
        if args.show_layers:
            ...
            for entity in entities:
                if hasattr(entity, 'layer'):
                    ...
                    layer_stats[layer]['types'][etype] = layer_stats[layer]['types'].get(etype, 0) + 1
            ...
            for layer_name, stats in sorted_layers:
                ...
                # 显示每个图层中的实体类型
                type_info = ", ".join([f"{etype}: {count}" for etype, count in stats['types'].items()])
        ...
        if args.layer:
            ...
            if not filtered_entities and not filtered_blocks:
                ...
                print(f"已过滤，仅显示图层 '{args.layer}' 中的 {len(filtered_entities)} 个实体和 {len(filtered_blocks)} 个块")
    ...
    if args.view_range:
        try:
            ...
            if len(focus_area) != 4:
                print("警告: 视图范围格式错误，应为'min_x,min_y,max_x,max_y'，使用自动范围")
                focus_area = None
            else:
