    ...
    class OCCBackend(STEPBackendBase):
            ...
            for i in range(1, self.doc.NbEntities() + 1):
                ...
                if entity.IsKind("SHAPE"):
                    shape = {
                        ...
                        "name": (
                            entity.Name().ToCString() if hasattr(entity, "Name") else ""
                        ),
                    }
                    shapes.append(shape)
                ...
                if entity.IsKind("NEXT_ASSEMBLY_USAGE_OCCURRENCE"):
                    if str(entity.RelatingProduct().GetHashCode()) == assembly_id:
                        component = {
                            ...
                            "name": (
                                entity.RelatedProduct().Name().ToCString()
                                if hasattr(entity.RelatedProduct(), "Name")
                                else ""
                            ),
        ...
        def get_edges(self, shape_id: str) -> List[Dict]:
            ...
            while explorer.More():
                ...
                # 获取起点和终点
                p1 = BRep_Tool.Pnt(
                    topods_Vertex(TopExp_Explorer(edge, TopAbs_VERTEX).Current())
                )
                ...
                if curve_type == GeomAbs_Line:
                    ...
                    edge_data = {
                        ...
                        "type": (
                            "CIRCLE"
                            if curve_adaptor.LastParameter()
        ...
        def get_shapes(self) -> List[Dict]:
            ...
            for obj in self.doc.Objects:
                if hasattr(obj, "Shape"):
                    shape = {"id": str(obj.Name), "type": obj.TypeId, "name": obj.Label}
                    shapes.append(shape)
        ...
        def get_assemblies(self) -> List[Dict]:
            ...
            for obj in self.doc.Objects:
                if hasattr(obj, "Group") and obj.Group:
                    assembly = {"id": str(obj.Name), "name": obj.Label}
                    assemblies.append(assembly)
                    ...
                    if len(parts) >= 2:
                        ...
                        if type_info.startswith("ADVANCED_FACE"):
                            ...
                            self.assemblies.append(
                                {"id": current_id, "name": f"Assembly_{current_id[1:]}"}
                            )
    ...
    def parse_file(
        ...
        try:
                ...
                for component in components:
                    ...
                    if block:
                        ...
                        transformed_entities = self._transform_entities(
                            block.entities, translation, rotation, scale
                        )
            ...
            return (
                ...
                {"shapes": shapes, "assemblies": assemblies},
            )
