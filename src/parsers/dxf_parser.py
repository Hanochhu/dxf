    ...
    class EzdxfBackend(DXFBackendBase):
        ...
        def get_basic_entities(self) -> List[Dict]:
            ...
            for entity in self.modelspace:
                ...
                if (
                    entity_dict and entity_dict.get("type") != "INSERT"
                ):  # 过滤掉INSERT实体
        ...
        def get_blocks(self) -> Dict[str, Dict]:
            ...
            for block in self.doc.blocks:
                blocks[block.name] = {
                    ...
                    "base_point": (
                        (0, 0, 0)
                        if not hasattr(block.dxf, "base_point")
                        else tuple(block.dxf.base_point)
                    ),
                }
        ...
        def get_attributes(self, owner_handle: str) -> List[Dict]:
            ...
            for entity in self.modelspace:
                if (
                    ...
                    and hasattr(entity.dxf, "owner")
                    and entity.dxf.owner == owner_handle
                ):
            ...
            try:
                if entity_type == "LINE":
                    ...
                    return {
                        ...
                        "layer": entity.dxf.layer,
                        "points": [
                            tuple(getattr(entity.dxf, f"vtx{i}")) for i in range(4)
                        ],
                    }
    ...
    def _parse_blocks_section(self, contents, start_index):
        ...
        while i < len(contents):
            ...
            if line == "BLOCK":
                ...
                current_block = None
                i += 1
            elif current_block is not None and line in (
                "LINE",
                "CIRCLE",
                "ARC",
                "TEXT",
...
class DXFParser(CADFileParser):
        ...
        try:
            if entity_type == "LINE":
                ...
                return TextEntity(
                    ...
                    entity_type=(
                        EntityType.TEXT if entity_type == "TEXT" else EntityType.MTEXT
                    ),
                    layer=entity_dict.get("layer", ""),
                ...
                if entity_type == "LWPOLYLINE":
                    return LwPolylineEntity(
                        id=entity_dict.get(
                            "handle", self.generate_unique_id("LWPOLYLINE_")
                        ),
                    ...
                    return PolylineEntity(
                        id=entity_dict.get(
                            "handle", self.generate_unique_id("POLYLINE_")
                        ),
                ...
                return LeaderEntity(
                    ...
                    layer=entity_dict.get("layer", ""),
                    vertices=[
                        Point.from_tuple(v) for v in entity_dict.get("vertices", [])
                    ],
                )
            elif entity_type == "SOLID":
                return SolidEntity(
                    ...
                    layer=entity_dict.get("layer", ""),
                    points=[
                        Point.from_tuple(pt) for pt in entity_dict.get("points", [])
                    ],
                )
            elif entity_type == "POINT":
                return PointEntity(
    ...
    def _transform_entity_dict(
        self, entity_dict: Dict, position: Tuple, scale: Tuple, rotation: float
    ) -> Dict:
        """变换实体字典"""
        entity_type = entity_dict.get("type", "")
