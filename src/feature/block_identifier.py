...
class BlockFeatureExtractor:
    ...
    def extract_feature_dict(self, block: Block) -> Dict:
        ...
        entity_types = {
            etype.value: count for etype, count in type_counts.items() if count > 0
        }
    ...
    def analyze_block_pattern(self, block: Block) -> Dict:
        ...
        if (
            EntityType.CIRCLE.value in entity_types
            and entity_types[EntityType.CIRCLE.value] > 0
        ):
            if (
                EntityType.LINE.value in entity_types
                and entity_types[EntityType.LINE.value] > 0
            ):
        ...
        if block.is_arrow:
            ...
            if enhanced_features["composite_pattern"] == "circle_with_lines":
                ...
                if (
                    "shape_type" in enhanced_features
                    and enhanced_features["shape_type"] == "square"
                ):
...
class BlockIdentifier:
    ...
    def _calculate_match_score(self, block: Block, feature: BlockFeature) -> float:
        ...
        if count_range == 0:  # 避免除以零
            ...
            distance = min(
                abs(entity_count - feature.min_entity_count),
        ...
        if block.bounding_box:
            ...
            if ar_range == 0:  # 避免除以零
                ...
                ar_distance = min(
                    abs(aspect_ratio - feature.min_aspect_ratio),
        ...
        total_score = (
            ...
            * (
                1.0
                if all(rtype in block_entity_types for rtype in required_types)
                else 0.5
            )  # 类型匹配权重
    ...
    def save_templates(self, file_path: str):
        ...
        """
        templates = {
            name: feature.to_dict() for name, feature in self.block_features.items()
        }
...
class BlockClusterAnalyzer:
    ...
    def cluster_blocks(
        self, blocks: List[Block], n_clusters: int = 0, method: str = "kmeans"
    ) -> Dict[int, List[Block]]:
        """
        对块进行聚类
        ...
        # 提取特征
        features = np.array(
            [self.feature_extractor.extract_features(block) for block in blocks]
        )
        ...
        if n_clusters <= 0:
            ...
            n_clusters = self._estimate_clusters(
                features, max_clusters=min(10, len(blocks))
            )
...
class BlockLayoutAnalyzer:
    ...
    def analyze_spatial_distribution(self, blocks: List[Block]) -> Dict:
        ...
        """
        if not blocks:
            return {"empty": True, "message": "No blocks to analyze"}
        ...
        for block in blocks:
            if block.bounding_box and block.bounding_box.center:
                centers.append(
                    (block.bounding_box.center.x, block.bounding_box.center.y)
                )

        if not centers:
            return {"empty": True, "message": "No valid block centers found"}
        ...
        # 划分网格进行密度分析
        grid_size = 5  # 5x5网格
        grid_density = self._calculate_grid_density(
            centers, min_x, max_x, min_y, max_y, grid_size
        )
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                if grid_density[i][j] > density * 1.5:  # 密度高于平均值50%
                    ...
                    hotspots.append(
                        {"position": (grid_x, grid_y), "density": grid_density[i][j]}
                    )
        ...
        for start_idx in range(len(centers)):
            ...
            for angle, group in angle_groups.items():
                if len(group) >= 2:
                    ...
                    for i in range(len(distances) - 1):
                        if (
                            abs((distances[i + 1] - distances[i]) - avg_diff)
                            > avg_diff * 0.2
                        ):  # 20%的容差
            ...
            return [
                {
                    ...
                    "fill_rate": fill_rate,
                    "blocks": [
                        block for cell_blocks in grid.values() for block in cell_blocks
                    ],
                }
            ]
        ...
        for center_idx, center_point in enumerate(centers):
            ...
            for i, point in enumerate(centers):
                if i != center_idx:
                    ...
                    distance = math.sqrt(dx * dx + dy * dy)
                    points_with_angles.append(
                        {"block": point["block"], "angle": angle, "distance": distance}
                    )
            ...
            distance_variation = (
                distance_std / avg_distance if avg_distance > 0 else float("inf")
            )

            # 检查角度是否均匀分布
            angles = sorted([p["angle"] for p in points_with_angles])
            angle_diffs = [
                (angles[i + 1] - angles[i]) % (2 * math.pi)
            ...
            angle_std = (
                math.sqrt(
                    sum((d - avg_angle_diff) ** 2 for d in angle_diffs)
                    / len(angle_diffs)
                )
                if angle_diffs
                else 0
            ...
            angle_variation = (
                angle_std / avg_angle_diff if avg_angle_diff > 0 else float("inf")
            )

            # 判断是否构成放射状模式
            if (
                distance_variation < 0.3  # 距离变异系数小于30%
                and angle_variation < 0.3  # 角度变异系数小于30%
                and len(points_with_angles) >= 3
            ):  # 至少有3个辐射点

                blocks_in_pattern = [center_point["block"]] + [
                    p["block"] for p in points_with_angles
                ]
