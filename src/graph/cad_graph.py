...
class CADGraph:
    ...
    def get_predecessors(self, block_id: str) -> List[Block]:
        ...
        return [self.blocks[pred_id] for pred_id in predecessors if pred_id in self.blocks]
    ...
    def get_successors(self, block_id: str) -> List[Block]:
        ...
        return [self.blocks[succ_id] for succ_id in successors if succ_id in self.blocks]
    ...
    def get_all_paths(self, source_id: str, target_id: str, cutoff: int = None) -> List[List[str]]:
        """
        查找从源块到目标块的所有路径
    ...
    def get_central_blocks(self, top_n: int = 5, method: str = 'degree') -> List[Tuple[str, float]]:
        """
        获取图中最中心的块
        ...
        try:
            ...
            for block_id, block in self.blocks.items():
                ...
                if block.bounding_box:
                    block_data['bounding_box'] = {
                        'min': [block.bounding_box.min_point.x, block.bounding_box.min_point.y, block.bounding_box.min_point.z],
                        'max': [block.bounding_box.max_point.x, block.bounding_box.max_point.y, block.bounding_box.max_point.z],
            ...
            for conn_id, conn in self.connections.items():
                ...
                for segment in conn.path_segments:
                    segment_info = {
                        'id': segment.id,
                        'start': [segment.start_point.x, segment.start_point.y, segment.start_point.z],
                        'end': [segment.end_point.x, segment.end_point.y, segment.end_point.z],
        ...
        try:
            ...
            for source, target in data.get('graph', {}).get('edges', []):
                ...
                for conn_id_check, conn_data in data.get('connections', {}).items():
                    if conn_data.get('source_block_id') == source and conn_data.get('target_block_id') == target:
    ...
    def get_statistics(self) -> Dict:
        ...
        stats = {
            ...
            'arrow_count': sum(1 for _, data in self.graph.nodes(data=True) if data.get('is_arrow', False)),
            'directed_connection_count': sum(1 for _, _, data in self.graph.edges(data=True) if data.get('explicit_direction', False)),
            'indirect_connection_count': sum(1 for _, _, data in self.graph.edges(data=True) if data.get('connection_type', '') == 'indirect'),
        ...
        if segment_counts:
            stats['average_connection_segments'] = sum(segment_counts) / len(segment_counts)
...
class GraphVisualizer:
    ...
    def generate_layout(self, algorithm: str = 'spring') -> Dict[str, Tuple[float, float]]:
        """
        生成图的布局（节点位置）
    ...
    def use_physical_positions(self) -> Dict[str, Tuple[float, float]]:
        ...
        for node_id, node_data in self.cad_graph.graph.nodes(data=True):
            if 'center_x' in node_data and 'center_y' in node_data:
                ...
                positions[node_id] = (node_data['ref_position_x'], node_data['ref_position_y'])
            elif 'bbox_min_x' in node_data and 'bbox_min_y' in node_data and 'bbox_max_x' in node_data and 'bbox_max_y' in node_data:
                # 使用边界框中心点
        ...
        missing_nodes = [node for node in self.cad_graph.graph.nodes() if node not in positions]
        if missing_nodes:
            # 创建子图并计算布局
    ...
    def get_node_colors(self, attribute: str = 'is_arrow', default_color: str = '#1f77b4') -> Dict[str, str]:
        """
        生成节点颜色映射
        ...
        for node_id, node_data in self.cad_graph.graph.nodes(data=True):
            if attribute in node_data:
                ...
                # 针对数值类型属性（例如 'entity_count'）
                    ...
                    # 将值归一化到0-1范围
                    min_val = min(node_data.get(attribute, 0) for _, node_data in self.cad_graph.graph.nodes(data=True) if attribute in node_data)
                    max_val = max(node_data.get(attribute, 0) for _, node_data in self.cad_graph.graph.nodes(data=True) if attribute in node_data)
    ...
    def get_edge_colors(self, attribute: str = 'explicit_direction', default_color: str = '#000000') -> Dict[Tuple[str, str], str]:
        """
        生成边颜色映射
        ...
        for source, target, edge_data in self.cad_graph.graph.edges(data=True):
            if attribute in edge_data:
                ...
                # 针对数值类型属性（例如 'segments'）
                    ...
                    # 将值归一化到0-1范围
                    min_val = min(edge_data.get(attribute, 0) for _, _, edge_data in self.cad_graph.graph.edges(data=True) if attribute in edge_data)
                    max_val = max(edge_data.get(attribute, 0) for _, _, edge_data in self.cad_graph.graph.edges(data=True) if attribute in edge_data)
